...
public class VerifyPropertyIsLikeOperatorTests {
	...
	public void generateStringPatternForLocationNameInSimpleFeature() {
		List<String> valueList = Arrays.asList("Haida Gwaii", "Queen Charlotte Islands");
		when(suite.getAttribute(org.opengis.cite.iso19136.SuiteAttribute.XSMODEL.getName())).thenReturn(model);
		when(dataSampler.getSimplePropertyValues(any(QName.class), any(QName.class), any())).thenReturn(valueList);
		...
		Map<QName, String> prop = iut.generateMatchingStringPattern(featureType);
		Assert.assertFalse("Expected to find string property value pattern for SimpleFeature.", prop.isEmpty());
		Entry<QName, String> entry = prop.entrySet().iterator().next();
		Assert.assertEquals("Unexpected property name", "locationName", entry.getKey().getLocalPart());
		Assert.assertEquals("Unexpected pattern.", "*ida Gwaii", entry.getValue());
	}
	...
	public void generateStringPatternForLocationCodeInComplexFeature() {
		...
		iut.initQueryFilterFixture(testContext);
		Map<QName, String> prop = iut.generateMatchingStringPattern(featureType);
		Assert.assertFalse("Expected to find string property value pattern for ComplexFeature.", prop.isEmpty());
		Entry<QName, String> entry = prop.entrySet().iterator().next();
		Assert.assertEquals("Unexpected property name", "locationCode", entry.getKey().getLocalPart());
