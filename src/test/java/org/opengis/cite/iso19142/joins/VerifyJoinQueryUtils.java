...
public class VerifyJoinQueryUtils extends CommonTestFixture {
	...
	public static void buildSchemaModel() throws SAXException {
		...
		InputStream inStream = VerifyAppSchemaUtils.class.getResourceAsStream("/xsd/simple.xsd");
		Schema schema = xsdCompiler.compileXmlSchema(new StreamSource(inStream));
	...
	public void noFeatureProperties() {
		...
		Document reqEntity = WFSMessage.createRequestEntity("GetFeature-Minimal", "2.0.0");
		List<FeatureProperty> geomProps = null;
		JoinQueryUtils.appendSpatialJoinQuery(reqEntity, "Intersects", geomProps);
	}
	...
	public void buildIntersectsQuery() {
		...
		geomProps.add(
				new FeatureProperty(new QName(NS1, "SimpleFeature"), model.getElementDeclaration("lineProperty", NS1)));
		geomProps.add(new FeatureProperty(new QName(NS1, "ComplexFeature"),
				model.getElementDeclaration("multiGeomProperty", NS1)));
		JoinQueryUtils.appendSpatialJoinQuery(reqEntity, "Intersects", geomProps);
		NodeList valueRefs = reqEntity.getElementsByTagNameNS(FES, "ValueReference");
		assertEquals("Unexpected number of fes:ValueReference elements.", 2, valueRefs.getLength());
		Node valueRef = valueRefs.item(0);
		String xpath = valueRef.getTextContent();
		String propName = xpath.substring(xpath.lastIndexOf('/') + 1);
		assertEquals("Unexpected local name", "lineProperty", propName.split(":")[1]);
		assertEquals("Unexpected namespace name", NS1, valueRef.lookupNamespaceURI(propName.split(":")[0]));
	}
