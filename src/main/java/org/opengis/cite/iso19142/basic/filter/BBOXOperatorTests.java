...
public class BBOXOperatorTests extends QueryFilterFixture {
	...
	public void withBBOXFilter(ProtocolBinding binding, QName featureType) {
		...
		if (!geomProp.getNamespace().equals(featureType.getNamespaceURI())) {
			...
			xpath = String.format("//ns1:%s/ns2:%s/*[1]",
					featureType.getLocalPart(), geomProp.getName());
		} else {
			xpath = String.format("//ns1:%s/ns1:%s/*[1]",
					featureType.getLocalPart(), geomProp.getName());
		}
		NodeList geometryNodes;
		try {
			geometryNodes = XMLUtils.evaluateXPath(this.rspEntity, xpath,
					nsBindings);
		} catch (XPathExpressionException e) {
			throw new RuntimeException(e);
		}
		Assert.assertTrue(geometryNodes.getLength() > 0, ErrorMessage.format(
				ErrorMessageKeys.XPATH_RESULT, this.rspEntity
						.getDocumentElement().getNodeName(), xpath));
		...
		for (int i = 0; i < geometryNodes.getLength(); i++) {
			...
			if (geometry.getElementsByTagNameNS(Namespaces.GML, "PolygonPatch")
					.getLength() > 0) {
			...
			try {
				...
				Assert.assertTrue(intersects, ErrorMessage.format(
						ErrorMessageKeys.PREDICATE_NOT_SATISFIED, "BBOX",
						XMLUtils.writeNodeToString(gmlPolygon),
						XMLUtils.writeNodeToString(geometry)));
			} catch (TransformException te) {
				throw new RuntimeException("Coordinate transformation failed.",
						te);
			}
		}
	}
	...
	Element surfaceToPolygon(Element geometry) {
		Document result = XMLUtils.transform(new StreamSource(getClass()
				.getResourceAsStream("surface2polygon.xsl")), geometry);
