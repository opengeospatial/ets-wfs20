...
public class LockingCapabilitiesTests extends BaseFixture {
	 ...
	 * {@value org.opengis.cite.iso19142.WFS2#LOCKING_WFS} has the value 'TRUE'. If not,
	 * all tests of locking behavior will be skipped.
	...
	public void implementsLockingWFS(ITestContext testContext) {
		...
		if (!xpathResult) {
			throw new SkipException(
					"The service don't have the " + WFS2.LOCKING_WFS + " implementation so tests are skipped.");
		}
	}

	/**
	 * Builds a DOM Document representing a GetCapabilities request for a complete service
	 * metadata document.
	 */
	@BeforeClass
	...
	/**
	 * [{@code Test}] Checks the content of the complete service metadata document for
	 * additional service endpoints and properties (constraints) that must be present. The
	 * applicable rules are incorporated into the {@value #LOCKING_WFS_PHASE} phase of the
	 * Schematron schema {@code wfs-capabilities-2.0.sch}.
	 *
